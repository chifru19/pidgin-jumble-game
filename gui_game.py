...
class PidginScrabble:
    def __init__(self, root, player_name):
        ...
        self.submit_btn = tk.Button(root, text="SUBMIT WORD", command=self.check_word, bg="#f0f0f0", 
                                   fg="black", font=("Arial", 12, "bold"), height=2, width=22)
    ...
    def next_round(self):
        ...
        # Difficulty Logic: Word letters + 2 random distractors
    ...
    def refresh_tiles(self):
        ...
        # Inner frame to keep tiles perfectly centered
    ...
    def give_hint(self):
        if self.score >= 5:
            ...
            messagebox.showwarning("Locked", "Earn 5 points to unlock a hint!")
    ...
    def check_word(self):
        ...
        if user_word == self.target_word:
            ...
            self.score += 10
            self.update_ui()
            messagebox.showinfo("✅ Correct", f"Great job! '{self.target_word}' is correct.")
            self.next_round()
        else:
            self.wrong_answer()
    ...
    def update_ui(self):
        ...
        if self.score >= 5:
            self.hint_btn.config(bg="#28a745", fg="white", cursor="hand2")
        else:
            self.hint_btn.config(bg="#555", fg="white", cursor="arrow")
    ...
    def update_leaderboard(self):
        ...
        if self.score > current_top:
            ...
            messagebox.showinfo("🏆 New Record!", f"Incredible! {self.score} is the new record!")
        else:
...
if __name__ == "__main__":
    root = tk.Tk()
    root.withdraw()
    name = simpledialog.askstring("Login", "Enter Player Name:")
